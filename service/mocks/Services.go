...
func (_m *Services) CheckAvailability(ctx context.Context, id int, date string) ([]domain.Slot, error) {
	ret := _m.Called(ctx, id, date)

	var r0 []domain.Slot
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, int, string) ([]domain.Slot, error)); ok {
		return rf(ctx, id, date)
	}
	if rf, ok := ret.Get(0).(func(context.Context, int, string) []domain.Slot); ok {
		r0 = rf(ctx, id, date)
	} else {
		if ret.Get(0) != nil {
