...
func InitRouter(deps dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/user/venues/{venue_id}", authMiddleware(service.GetVenue(deps.CustomerServices))).Methods(http.MethodGet)
	...
	router.HandleFunc("/user/venues/{venue_id}/slots", authMiddleware(service.CheckAvailability(deps.CustomerServices))).Methods(http.MethodGet)
