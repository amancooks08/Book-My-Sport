...
func InitRouter(deps dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/venue_owner/venues/{venueID}", authMiddleware(service.DeleteVenue(deps.CustomerServices))).Methods(http.MethodDelete)
	router.HandleFunc("/user/venues/{venueID}/slots", authMiddleware(service.CheckAvailability(deps.CustomerServices))).Methods(http.MethodGet)
	router.HandleFunc("/customer/venues/{venueID}/book", authMiddleware(service.BookSlot(deps.CustomerServices))).Methods(http.MethodPost)
