...
func InitRouter(deps dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/admin/register", service.RegisterAdmin(deps.CustomerServices)).Methods(http.MethodPost)
	router.HandleFunc("/user/login", service.LoginUser(deps.CustomerServices)).Methods(http.MethodPost)
	router.HandleFunc("/admin/venues/add", authMiddleware(service.AddVenue(deps.CustomerServices))).Methods(http.MethodPost)
	...
	router.HandleFunc("/admin/venues/{venue_id}", authMiddleware(service.UpdateVenue(deps.CustomerServices))).Methods(http.MethodPut)
	router.HandleFunc("/admin/venues/{venue_id}", authMiddleware(service.DeleteVenue(deps.CustomerServices))).Methods(http.MethodDelete)
